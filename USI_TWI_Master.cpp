...
void USI_TWI_Master_Initialise( void )
{
  ...
  PORT_USI_SCL |= (1<<PIN_USI_SCL);           // Enable pullup on SCL, to set high as released state.
  
  DDR_USI_SCL  |= (1<<PIN_USI_SCL);           // Enable SCL as output.
...
/*Write address and Read/Write data */
  do
  {
    ...
    {
      /* Write a byte */
      PORT_USI_SCL &= ~(1<<PIN_USI_SCL);                // Pull SCL LOW.
...
unsigned char USI_TWI_Master_Transfer( unsigned char temp )
{
  ...
  { 
    ...
    while( !(PIN_USI_SCL & (1<<PIN_USI_SCL)) );// Wait for SCL to go high.
	_delay_us(T4_TWI);
...
/* Release SCL to ensure that (repeated) Start can be performed */
  PORT_USI_SCL |= (1<<PIN_USI_SCL);                     // Release SCL.
  while( !(PORT_USI_SCL & (1<<PIN_USI_SCL)) );          // Verify that SCL becomes high.
  _delay_us(T2_TWI);
